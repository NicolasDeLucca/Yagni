...
async function setToken() {
    try {
        ...
        console.log('Kitchen App - Successful connection with IDP.'); 
        logger.info('Kitchen App - Successful connection with IDP', { token });
        return token;
    } catch (error) {
        logger.error(`Kitchen App - Connection with IDP Failed: ${error}`);
        console.error('Kitchen App - Connection with IDP Failed.');
    }
}
